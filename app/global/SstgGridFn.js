Ext.define("krf_new.global.SstgGridFn", {
    ...
    me: null,
    basicComboStore: null,
    sstgDetailWindow: null,
    sstgDetailCombo: null,
    ...
    comboConfig: {
        ...
        '3': { combo: { 'sstgDetailArea2': { isView: false }, 'sstgDetailArea3': { isView: false } } },
        '4': { combo: { 'sstgDetailArea2': { isView: true }, 'sstgDetailArea3': { isView: false } } },
        '5': { combo: { 'sstgDetailArea2': { isView: false }, 'sstgDetailArea3': { isView: true } } }
    },
    ...
    getEsstgHcBemaSe: function (val) {
        ...
        if (val == 1) {
            grid = [{
                ...
                text: '조사방법',
                columns: [{
                    text: 'Surber net(80*80)',
                    dataIndex: 'SBN80_EXAMIN_CO',
                    width: 100
                }, {
                    text: 'Surber net(80*80)',
                    dataIndex: 'SBN80_EXAMIN_CO',
    ...
    sstgDetailInit: function () {
        ...
        me.sstgDetailWindow.show();

        me.setSstgDetailCombo();
    },

    setSstgDetailCombo: function () {
        me.sstgDetailCombo = Ext.getCmp('sstgDetailCombo');

        me.basicComboStore = Ext.create('Ext.data.Store', {
            fields: ['id', 'name'],
            data: [{ id: 0, name: '선택하세요.' }
                , { id: '1', name: '건강성 평가 지수 변경 이력' }
                , { id: '2', name: '조사지점 변경 이력' }
                , { id: '3', name: '조사항목 변경 이력' }
                , { id: '4', name: '조사자료 변경 이력' }
                , { id: '5', name: '생물종 특성 변경 이력' }
            ]
        });

        me.sstgDetailCombo.setStore(me.basicComboStore);
        me.sstgDetailCombo.setValue(0);
    },

    sstgDetailExtendInit: function (val) {

        me.sstgDetailWindow.hide();

        me.sstgDetailExtendWindow = Ext.getCmp('sstgDetailExtendWindow');
        if (!me.sstgDetailExtendWindow) {
            me.sstgDetailExtendWindow = Ext.create('krf_new.view.center.SstgDetailExtendWindow');
            Ext.getCmp('center_container').add(me.sstgDetailExtendWindow);
        }

        me.sstgDetailExtendWindow.show();

        me.sstgDetailExtendCombo = Ext.getCmp('sstgDetailExtendCombo');

        me.sstgDetailExtendCombo.setStore(me.basicComboStore);
        me.sstgDetailExtendCombo.setValue(val);

        me.setExtendCombo(val);
    },

    setExtendCombo: function (val) {
        var config = me.comboConfig[val].combo;
