	...
	selectUpLine: function (curRchDid, dnGeoTrib, drawOption, cnt) {
		...
		require(["esri/tasks/query",
			...
			"esri/geometry/Extent"], function (Query, QueryTask, Point, Extent) {
				...
				////console.info($KRF_DEFINE.reachServiceUrl_v3 + "/" + _reachLineLayerId);
	...
	selectNodeWithEvent: function (evt, drawOption) {
		...
		require(["esri/tasks/query",
			...
			"esri/geometry/Extent"], function (Query, QueryTask, Point, Extent) {
				...
				var queryTask = new QueryTask($KRF_DEFINE.reachServiceUrl_v3 + "/" + $KRF_DEFINE.reachNodeLayerId); // 리치노드 URL
				var query = new Query();
				query.returnGeometry = true;
				query.outFields = ["*"];
